...
class MultiStageFilter:
    def __init__(self, device=None):
        ...
        self.training_data = self.load_training_data('training_data/training_data.json')
    ...
    def load_training_data(self, file_path):
        try:
            ...
            return {"narrow_focus": [], "wide_focus": []}

    def gross_filter(self, answer):
        cleaned_answer = self.regex_cleaner.clean_text(answer, stage='gross')
        if len(cleaned_answer) == 0:
            logger.error("Answer is empty after gross filtering.")
            return answer
        return cleaned_answer

    def moderate_filter(self, answer):
        moderately_cleaned_answer = self.regex_cleaner.clean_text(answer, stage='moderate')
        if len(moderately_cleaned_answer) == 0:
            logger.error("Answer is empty after moderate filtering.")
            return answer
        return moderately_cleaned_answer
    ...
    def enhance_answer(self, query, original_answer):
        ...
        try:
            ...
            cleaned_answer = self.gross_filter(original_answer)

            # Stage 2: Moderate filtering
            moderately_cleaned_answer = self.moderate_filter(cleaned_answer)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Multi-Stage Answer Enhancement Script")
